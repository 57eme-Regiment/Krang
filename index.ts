...
const parseSpecialPage = ($: cheerio.CheerioAPI): [string[], string | undefined] => {
	const pageUrls: string[] = [];

	const nav = $("div.mw-allpages-nav");
	const nextPageLink = nav.children().last().text().includes("Next page") === false ? undefined : nav.children().last().attr('href');
	...
	pageList.children().each((index, element) => {
		...
		if (pageUrl) {
			...
			console.warn(`No href found for element at index ${index} : ${$(element).text()}`);
		}
	});
...
const getItemInfo = async (url: string) => {
	try {
		...
		const chassis = infoBox.find('div[data-source="ChassisName"]').children().last().text();
		const type = infoBox.find('div[data-source="type"]').children().last().text();
		const crateAmount = infoBox.find('div[data-source="crate_amount"]').children().last().text();
